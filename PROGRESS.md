...
v3 — Rust 🦀  █░░░░░░░░░    ~8%   started (M5) — model layer: one Rust module per IR module (collision-safe across API versions, D-147); serde structs, `Option<Option<T>>` tri-state via `double_option`, open enums as `String` newtypes + consts, closed enums, aliases; rustfmt-clean by construction + `cargo check`/`clippy -D warnings` gate on the full spec (VR-1.2). Next: typed serde-tagged unions (TR-Rust.1), async managers/client, `reqwest`/`tokio` runtime (TR-Rust.5), tests + docs, CLI/CI wiring
...
| **M5** — Rust backend + runtime (TR-Rust, VR-1.2) | serde-tagged enums, `Result`/`Option`, async reqwest/tokio runtime | 🔄 ~8% — model layer: module-per-IR-module (collision-safe across API versions), serde structs, `Option<Option<T>>` tri-state, open-enum `String` newtypes, closed enums, aliases; rustfmt-clean by construction + `cargo check`/`clippy` gate on the full spec (VR-1.2, D-147). Next: typed serde-tagged unions, async managers/client + `reqwest`/`tokio` runtime, tests + docs |
...
as real `enum`s, aliases, and `buildinfo` provenance. It's rustfmt-clean by
construction and passes `cargo check` + `clippy -D warnings` on the full spec —
the VR-1.2 gate, enforced by a backend test that runs the real toolchain. Next
slices: typed serde-tagged unions with unknown-discriminator retention
(TR-Rust.1), the async managers/client with builders (TR-Rust.3), the
hand-written `reqwest`/`tokio` runtime (TR-Rust.5), generated tests + docs, and
the CLI (`--target rust`) + `ci.yml` wiring. **Then M6 — TypeScript (v4)**
