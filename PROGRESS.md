...
(again, no completed work lost). **v4 (TypeScript) is now capability-complete**:
model layer, managers/client, `fetch` runtime (all four auth flows), pagination,
reference docs, and generated behavioral tests all landed, so
**`conform --target typescript` reads 9/9 and gates CI** alongside Go/Apex/Rust
(D-166) — only the NF-8 npm ship artifact and VR-7 live smoke remain for a
shipped v4. The large shared cost — the whole
...
| v3 — Rust 🦀 | 380 | 132 | ~72% |
| v4 — TypeScript 🟦 | 320 | 96 | ~68% |
| v5 — Java 26 ☕ | 360 | 108 | 0% |
| **Total** | **3,790** | **1,160** | **~84%** |
