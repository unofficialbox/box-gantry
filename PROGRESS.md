...
traceability pass today; docs, generated tests, and pagination surfaces are the
remaining shortfalls, so it's an honest progress report (no faked exclusions)
that joins the release gate once those land. Next slices: pagination `Stream`s,
typed date/time, and generated tests + docs (which flip `conform --target rust`
green). **Then M6 — TypeScript (v4)**
