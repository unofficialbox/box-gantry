...
/// reference docs.
fn readme() -> String {
    // The banner is an absolute URL, not the relative `assets/banner.svg`
    // every other generated file in this repo would use — deliberately.
    // npm's website resolves a relative image path in a README against
    // `package.json`'s `repository.url`, which is pinned to box-gantry (not
    // this repo) for the OIDC trusted-publishing provenance check above. A
    // relative path here would "work" by accident, resolving to box-gantry's
    // own (different) banner at the same relative path instead of failing
    // loudly. An absolute URL sidesteps that rewrite entirely and renders
    // identically on GitHub.
    r##"<!-- Generated by box-gantry. DO NOT EDIT — regenerate from the specs instead. -->
![Box Open SDK for TypeScript](https://raw.githubusercontent.com/unofficialbox/box-open-ts-sdk/main/assets/banner.svg)
