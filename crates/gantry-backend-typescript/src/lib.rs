...
pub fn generate(
    ...
    let mut files = vec![
        ...
        // managers type-check against exactly the declared surface, without the
        // real runtime (FR-5.3). The real `fetch` runtime implements the same
        // contract (TR-TS.5, a later slice).
        GeneratedFile {
            path: "src/runtime.ts".to_string(),
            content: gantry_contract::typescript_stubs(&gantry_contract::V1, manifest),
        },
        GeneratedFile {
